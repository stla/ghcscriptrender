...
scriptToHtmlWithOutputs hsfile fragment packages = 
  do
    ...
    html0 <- getHsColourHtml "/tmp/hsfile.hs"
    ...
    let htmlOut = if fragment 
                     ...
                     else SU.replace "<title>/tmp/hsfile.hs" ("<title>" ++ basefilename) htmlWithInsertions
...
scriptToHtmlWithMonoOutputs hsfile fragment packages = 
  do
    ...
    case fragment of
         ...
         False -> putStrLn $ SU.replace "<title>/tmp/hsfile.hs" ("<title>" ++ basefilename) htmlWithInsertions
                    where basefilename = last $ SU.split "/" hsfile
